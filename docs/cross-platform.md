...
| `hiro_native_*` MCP 扩展层 | 本机自研 bridge，转调 Hermes `computer_use` 后端 | **大概率可**，但需目标机实调用验收 [待目标机验证] |
| 应用白名单（`capabilities.yaml` 的 `bundle_id`） | `com.apple.*` | **不可** — `bundle_id` 是 macOS 概念 |
| Safari 操作 | `com.apple.Safari` | **不可** — Windows 无 Safari |
| 浏览器自动化（Chromium 路线） | agent-browser + Chromium | **可** — 用 `hiro_native_browser_*` 而非 Safari 桌面路线 |
| 服务自启动 | macOS launchd `LaunchAgent` | **不可** — Windows 用计划任务 |
...
[已验证：来自 Hermes `tools/computer_use/cua_backend.py` 的安装说明]
...
[已验证：`cua-driver --help` 中 `autostart` 一节标注 **Windows-only today**]
...
| 就绪判据 | 两项 TCC 授权齐全 | 驱动健康即可 | 驱动健康即可 |
| 可能弹窗 | 系统设置里的授权对话框 | UIAccess worker `cua-driver-uia.exe` 可能触发 SmartScreen | 无 |
| 授予命令 | `cua-driver permissions grant` | 不适用 | 不适用 |

要点：
- **`SKILL.md` 第 D 节整节是 macOS TCC 内容，Windows 上跳过**，只需确认 `cua-driver doctor` 全绿。
- Linux 走 X11 / XWayland 辅助控制栈；纯 Wayland 支持程度需在目标机实测。
- Windows 的 UIAccess 相关限制（能否操作提权窗口等）**不要靠推测**，用 `cua-driver doctor --json` 的真实输出判断。
...
3. 白名单的**哈希锁机制本身是可移植的**：`capabilities.yaml` + `approved.sha256` +
   `approval.json` 三者哈希一致才放行（`app_policy.py` 的逻辑与 OS 无关）。
   移植时保留这套机制，只是把里面的应用条目换掉。

### 2.5 浏览器路线

Windows 上没有 Safari，**桌面 Safari 分支整段作废**，改用：

- `hiro_native_browser_*` 系列（navigate / snapshot / click / type / scroll / back / press /
  get_images / screenshot）——这条路走 Chromium 系，与 OS 无关。
...
以下是本仓库/本机服务的真实内容，`grep` 全文可复现 [已验证]：
...
| LaunchAgent plists | `local.hermes-gpt.{server,actions,tunnel-login}.plist` | 全部作废，改用 `cua-driver autostart` / 计划任务 |
...
2. `irm .../install.ps1 | iex` 装 `cua-driver`，跑 `cua-driver doctor --json` 到全绿。
3. `cua-driver autostart enable` 注册登录自启。
...
- [ ] `cua-driver --version` / `doctor --json` / `permissions status` 三项输出留档
